...
const RenderParsedHTML = ({
  ...
  return (
    <div className={cn('relative mb-10 sm:mb-0 min-w-0 w-full', containerClassName)}>
      <div
        className={cn(
          'overflow-hidden transition-all duration-300 ease-in-out w-full min-w-0',
          ENABLE_SHOW_MORE && isShrinked && !isExpanded && isOverflowing && 'h-fit max-h-[100px]'
        )}
      >
        <div
          ref={contentRef}
          className={cn(markdownStyles, 'break-words [overflow-wrap:anywhere] min-w-0 w-full', contentClassName)}
